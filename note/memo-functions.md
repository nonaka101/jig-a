...
> [!CAUTION]
> `{inputElement}.pattern = "\d{4}-\d{2}-\d{2}"` だとエスケープが自動処理されてしまうので、`String.raw` 等を使い生の文字列にする必要がある。
...
今回は（エスケープ処理を考えない）**ベースとなる処理** と、それをラップする形で**エスケープ処理付きの処理**で設計した。ベースとなる処理に渡す際には、`|` を `@@PIPE@@` など通常使われえない文字列へと退避する形で置き換え、処理の完了後に `\|` といったエスケープシーケンスを使って再置換する。
...
- バリデーションエラーの時の対応
- `input[type="date"]` が未対応時の対策（これは[全体に関係するもの](#全体に関係するもの)に記載）
...
> [!TIP]
> 調べてみると、末日付近での月数計算も一定のルールがあり実装はできそうである。今の段階では保留とする。
...
そして音の設定を切っている場合は、スクリーンリーダー用のクラス `hp_srOnly` をつけた `output` 要素に、一瞬だけ時刻テキスト付きの `span` を付与（付与した後にすぐ削除）している。  
これにより 必要な情報を読み上げた上で、それが残って現在時間と勘違いされることはないはず。
